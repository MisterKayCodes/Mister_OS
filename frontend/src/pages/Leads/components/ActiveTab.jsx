...
import { Bot, MessageSquare, Check, X, AlertTriangle, Users, Sparkles } from 'lucide-react';
import { updateLeadApi, approveDraftApi, deleteDraftApi, generateFollowupsApi } from '../../../utils/api';
...
export default function ActiveTab({ leads, drafts, token, onLeadsUpdate, onDraftsUpdate }) {
  ...
  const handleApproveDraft = async (draft) => {
    const finalContent = editedDrafts[draft.id] !== undefined ? editedDrafts[draft.id] : draft.content;
    try {
      await approveDraftApi(draft.id, finalContent, token);
      onDraftsUpdate(drafts.filter(d => d.id !== draft.id));
      setEditedDrafts(prev => { const n = { ...prev }; delete n[draft.id]; return n; });
    } catch (err) { alert(err.message); }
  };
  ...
  const handleGenerateFollowups = async () => {
    ...
    try {
      const res = await generateFollowupsApi(token);
      if (res.generated === 0) {
        alert(`No follow-ups generated. Either no leads are in "Follow-up" status, they've been messaged recently (< 3 days), or they already have a pending draft.`);
      } else {
        alert(`✅ ${res.generated} follow-up draft(s) generated! Check the Approval Inbox below.`);
        // Reload drafts
        const { fetchPendingDraftsApi } = await import('../../../utils/api');
        const newDrafts = await fetchPendingDraftsApi(token);
        onDraftsUpdate(newDrafts);
      }
    } catch (err) { alert(err.message); }
    finally { setGeneratingFollowups(false); }
  };
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="px-4 pt-2 pb-2 shrink-0 flex justify-end">
        <button
          onClick={handleGenerateFollowups}
          disabled={generatingFollowups}
          className="flex items-center gap-1.5 bg-purple-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-purple-700 transition disabled:opacity-50"
        >
          <Sparkles size={12} />
          {generatingFollowups ? 'Generating...' : '🤖 Generate Follow-ups'}
        </button>
      </div>
      ...
      <div className="flex flex-1 overflow-x-auto overflow-y-hidden p-4 gap-4 min-h-0">
        ...
        {/* Approval Inbox */}
        {drafts.length > 0 && (
          <div className="flex flex-col w-80 shrink-0 bg-white rounded-xl shadow-lg border border-purple-200 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-4">
              {drafts.map(draft => {
                ...
                const editedVal = editedDrafts[draft.id] !== undefined ? editedDrafts[draft.id] : draft.content;
                const isEdited = editedVal !== draft.content;
                return (
                  <div key={draft.id} className="bg-white border border-gray-200 rounded-lg p-3 flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <h4 className="font-semibold text-gray-800 text-sm">@{lead?.username || 'Unknown'}</h4>
                      {isEdited && (
                        <span className="text-[9px] bg-amber-100 text-amber-700 font-bold px-1.5 py-0.5 rounded-full">✏️ AI will learn</span>
                      )}
                    </div>
                    <textarea
                      value={editedVal}
                      onChange={e => setEditedDrafts(prev => ({ ...prev, [draft.id]: e.target.value }))}
                      className="w-full border border-gray-200 rounded-lg p-2 text-xs text-gray-700 focus:outline-none focus:border-purple-400 resize-none min-h-[80px] bg-gray-50"
                    />
                    <div className="flex justify-between items-center">
                      <button onClick={() => handleRejectDraft(draft.id)} className="p-1.5 text-red-400 hover:bg-red-50 rounded transition">
                        <X size={14} />
                      </button>
                      <button
                        onClick={() => handleApproveDraft(draft)}
                        className="flex items-center gap-1 bg-green-500 text-white px-3 py-1.5 rounded text-xs font-bold hover:bg-green-600 transition"
                      >
